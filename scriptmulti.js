...
document.addEventListener("DOMContentLoaded", function() {
  ...
  const playerCount = JSON.parse(localStorage.getItem("playerCount"));
  let playerIndex = 0;
  currentPlayer = playerNames[playerIndex];
  lastPlayer = playerNames[playerCount-1];
  document.getElementById("playername").innerText=currentPlayer+"'s Score: "
  const scoresheet = {};
  for(let i = 0; i<playerCount; i++) {
    scoresheet[playerNames[i]] = 0;
  }
  ...
  function gameOver() {
    ...
    revealFielderBlocks();
    scoresheet[currentPlayer] = score;
  ...
  function showPopupMessage() {
    ...
    if (score === 0) {
      const messages = [
        currentPlayer+": Ouch! Tough luck.",
        currentPlayer+": That was a duck out!",
        currentPlayer+": You seem to love scoring 0 ;)"
      ];
      const randomIndex = Math.floor(Math.random() * messages.length);
      popupMessage = messages[randomIndex];
    } else {
      popupMessage = currentPlayer+": Game over. Your score is " + score;
    }
